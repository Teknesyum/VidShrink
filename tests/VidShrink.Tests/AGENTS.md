...
- `WatchFolderTests.cs` — A3, `izle` alt komutu (`Core/WatchFolder.cs`): sahte dosya sistemi ve saatle kararlılık (boyut/zaman iki yoklamada sabit, oturma süresi, kilit), döngü koruması (`_shrunk` eki, aynı klasör reddi, durum dosyası), `.vidshrink-izle.json` yeniden başlatma ve bozuk dosya yedeği, hata veren dosyanın not edilip atlanması, Ctrl+C. Uçtan uca kol 2 sn 320x240 lavfi klibi `dotnet vidshrink.dll izle --bir-kez` ile küçültür, ffprobe okur. Kanıt `.calisma/test-ciktilari/hb-a3-izle/`, test siler.
