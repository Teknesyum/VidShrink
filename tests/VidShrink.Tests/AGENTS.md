...
- `WatchFolderTests.cs` — A3, `izle` (`Core/WatchFolder.cs`): sahte dosya sistemi ve saatle kararlılık (boyut/zaman iki ardışık aralıkta sabit, oturma süresi, kilit), kodlama sırasında büyüyen dosya (çıktı silinir, yeniden denenir), yalnız durumdaki çıktı adlarının atlanması, aynı klasör reddi ve Linux harf duyarlılığı, `.vidshrink-izle.json` yeniden başlatma, bozuk dosya yedeği, salt okunur klasörde çıktı/ayar klasörüne düşen durum, hatalı dosyanın bir kez yeniden denenmesi, Ctrl+C. CLI kolları 130/0/4 çıkışları. Uçtan uca kol 2 sn 320x240 lavfi klibi `dotnet vidshrink.dll izle --bir-kez --json` ile küçültür, NDJSON ve ffprobe okur. Kanıt `.calisma/test-ciktilari/hb-a3-izle/`, test siler.
