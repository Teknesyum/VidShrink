...
- `OynaticiSuruklemeTests.cs` — `OynaticiCiftTikSuresiTests` (`[Collection]` ile yalıtılmış `ClickArbiter.Source`): ham C/X ile 0.05 hız adımı motordan, çift tık süresi sistemden. `OynaticiSuruklemeTests`: kendi döngüde tuşsuz hareket ve yakalama kaybı pencereyi taşımaz; Windows yerel BeginMoveDrag, gerçek `SendMessage(WM_MOVING)` dikdörtgeni merkeze çekilir. Kanıt `.calisma/oynatici-kisayol/`.
