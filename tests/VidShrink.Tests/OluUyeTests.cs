...
public sealed class OluUyeTests
{
    ...
    private static readonly PinnedFinding[] Pinned =
    {
        ...
        new("RecordingImpact.SoftwareLightLoad", "varsayilan-kol", Legitimate,
            "Ayni olcum: Impact karar alani degil, rapor alani. PerformanceReportText mansetini PerformanceFindingCode uzerinden kuruyor."),
        new("ShareFailure.FileTooLarge", "varsayilan-kol", Legitimate,
            "K8 borcu 5 olculdu: ayrim enum uzerinden degil ShareDiagnosis.Key uzerinden kullaniciya ulasiyor (11 uyeye karsi 22 anahtar, hepsi 42 dilde pimli). Enum'un tasidigi karar yeniden deneme: ShareRetry.IsRetryable RateLimited/NetworkFailure/ServiceError'i adiyla soruyor, bu uye o sorunun olumsuz kolu. Tavani asan dosya yeniden denemekle gecmez; onun kolu SuggestedTargetId ile 'su hedefle dene' dugmesine gidiyor. PaylasimTekrarDenemeTests.KalanUyelerHicbirAdimdaYenidenDenenmiyor kolu pimliyor; kumeyi genisletmek kirmizi veriyor (docs/olcumler/k8-paylasim-tekrar-2026-09-19.md)."),
        new("ShareFailure.FileUnreadable", "varsayilan-kol", Legitimate,
            "K8 borcu 5 olculdu: ayrim enum uzerinden degil ShareDiagnosis.Key uzerinden kullaniciya ulasiyor (11 uyeye karsi 22 anahtar, hepsi 42 dilde pimli). Enum'un tasidigi karar yeniden deneme: ShareRetry.IsRetryable RateLimited/NetworkFailure/ServiceError'i adiyla soruyor, bu uye o sorunun olumsuz kolu. Okunamayan dosya ikinci denemede de okunmaz. PaylasimTekrarDenemeTests.KalanUyelerHicbirAdimdaYenidenDenenmiyor kolu pimliyor; kumeyi genisletmek kirmizi veriyor (docs/olcumler/k8-paylasim-tekrar-2026-09-19.md)."),
        new("ShareFailure.LocalDiskFull", "varsayilan-kol", Legitimate,
            "K8 borcu 5 olculdu: ayrim enum uzerinden degil ShareDiagnosis.Key uzerinden kullaniciya ulasiyor (11 uyeye karsi 22 anahtar, hepsi 42 dilde pimli). Enum'un tasidigi karar yeniden deneme: ShareRetry.IsRetryable RateLimited/NetworkFailure/ServiceError'i adiyla soruyor, bu uye o sorunun olumsuz kolu. Disk dolu kaldigi surece deneme ayni yere duser. PaylasimTekrarDenemeTests.KalanUyelerHicbirAdimdaYenidenDenenmiyor kolu pimliyor; kumeyi genisletmek kirmizi veriyor (docs/olcumler/k8-paylasim-tekrar-2026-09-19.md)."),
        new("ShareFailure.NotAuthorized", "varsayilan-kol", Legitimate,
            "K8 borcu 5 olculdu: ayrim enum uzerinden degil ShareDiagnosis.Key uzerinden kullaniciya ulasiyor (11 uyeye karsi 22 anahtar, hepsi 42 dilde pimli). Enum'un tasidigi karar yeniden deneme: ShareRetry.IsRetryable RateLimited/NetworkFailure/ServiceError'i adiyla soruyor, bu uye o sorunun olumsuz kolu. Jeton yanlissa ya da icerik engellendiyse tekrar gondermek ayni yaniti alir. PaylasimTekrarDenemeTests.KalanUyelerHicbirAdimdaYenidenDenenmiyor kolu pimliyor; kumeyi genisletmek kirmizi veriyor (docs/olcumler/k8-paylasim-tekrar-2026-09-19.md)."),
        new("ShareFailure.QuotaExceeded", "varsayilan-kol", Legitimate,
            "K8 borcu 5 olculdu: ayrim enum uzerinden degil ShareDiagnosis.Key uzerinden kullaniciya ulasiyor (11 uyeye karsi 22 anahtar, hepsi 42 dilde pimli). Enum'un tasidigi karar yeniden deneme: ShareRetry.IsRetryable RateLimited/NetworkFailure/ServiceError'i adiyla soruyor, bu uye o sorunun olumsuz kolu. Kota doldu; bekleme suresi sunucudan gelmiyor, yeniden deneme onerilmiyor. PaylasimTekrarDenemeTests.KalanUyelerHicbirAdimdaYenidenDenenmiyor kolu pimliyor; kumeyi genisletmek kirmizi veriyor (docs/olcumler/k8-paylasim-tekrar-2026-09-19.md)."),
        ...
        new("FfmpegArguments.SceneMapRuleOfRecord", "yalniz-disarida", Legitimate,
            "K8 borcu 12 kapandi: uretimde sifir gorunum kusur degil sartin kendisi. Bu alan uretimin varsayilan kuralinin (ThresholdRule.Measured) altin kopyasi; Az_bolme_duzeltmesi_olculdugu_kuralda_kalir ikisinin kesim listesini karsilastiriyor. Uretime indirilse ya da Measured'dan okusa olcu sabiti kendisiyle karsilastirir ve kural kaysa bile hep yesil kalirdi - kayit ancak olculenden bagimsiz durdugu surece kayittir. Bagimsizligin yuk tasidigi olculdu (docs/olcumler/k8-kayitli-kural-2026-09-19.md): Measured'in Slope'u kaydirilinca kayit pimi kiziriyor, kayit Measured'dan okuyacak hale getirilince ayni kaydirmada o pim susuyor. Kayma yine de gorunur kaliyor - SceneMapTests.TuretilenEsik_KiskacHerIkiUctaBaglar ikinci bir bagimsiz olcu - ama susan tam da bu alani ayakta tutan pimdir."),
        new("AudioCodecChoice.Auto", "varsayilan-kol", Legitimate,
            "B1a ses kodegi seciminin varsayilani: secim yok demek. PlanCalculator.PickAudioCodec Ac3 ve Eac3u adiyla yazip '_' koluna aac veriyor; Auto ile Aac ayni kodegi ureten iki giris, ayri kol ayni degeri yazardi."),
        new("AudioCodecChoice.Aac", "varsayilan-kol", Legitimate,
            "Ayni switchin ikinci varsayilan girisi; kullanici AACyi acikca secince Auto ile ayni kol calisiyor. Ayrim kullanicinin gordugu kutuda (MainWindow CmbAdvAudioCodec satir 1), motorda degil."),
        ...
        new("PresetKind.General", "varsayilan-kol", Legitimate,
            "K8 borcu 6 kapandi: CliApp.ProfilListesi dort turu bir tablodan gezip her birini OfKind ile suzuyor, uye tabloda adiyla yaziliyor. Adiyla karsilastiran kol yalniz User'da (kullanici profilleri dosyadan gelir, kutuphaneden degil); geri kalan uc uye dongu degiskeniyle tuketiliyor, o yuzden bicim borc degil varsayilan-kol. Ayrica adlandirmak ayni tabloyu iki kez yazardi."),
        new("PresetKind.Platform", "varsayilan-kol", Legitimate,
            "Ayni tablonun ikinci satiri; gerekcesi PresetKind.General ile ayni."),
        new("PresetKind.Device", "varsayilan-kol", Legitimate,
            "Ayni tablonun ucuncu satiri; gerekcesi PresetKind.General ile ayni."),
