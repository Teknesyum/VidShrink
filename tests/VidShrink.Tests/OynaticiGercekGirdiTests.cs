...
public sealed class OynaticiGercekGirdiTests
{
    ...
    /// </summary>
    private static void Kapat(params string[] adlar) => KanitKapanisi.Kapat(Path.Combine(GirdiKanit.Root, ".calisma", "girdi"), adlar);
    ...
    public void SeritDugmeleri_HamFareIle_MotoraUlasiyor()
    {
        ...
        var rapor = AppHost.Run(() =>
        {
            ...
            while (!view.SeritRevealed && saat.Elapsed.TotalSeconds < 5)
            {
                using var dilim = new System.Threading.CancellationTokenSource(TimeSpan.FromMilliseconds(2));
