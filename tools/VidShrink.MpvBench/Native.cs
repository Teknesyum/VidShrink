...
internal static unsafe class Native
{
    ...
    [DllImport(Lib, CallingConvention = CallingConvention.Cdecl)]
    public static extern int mpv_observe_property(IntPtr mpv, ulong replyUserdata,
    ...
    public static void Check(int code, string what)
    {
        if (code < 0)
        {
            throw new InvalidOperationException($"{what}: {Error(code)} ({code})");
        }
    }
}
