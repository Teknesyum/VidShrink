...
public static class PromptBuilder
{
    public static string Build(MediaInfo info, PlanOptions options, EncodePlan localPlan)
    {
        ...
        sb.AppendLine($"- duration: {Saat.Tani.Saniye(info.DurationSeconds)} s");
        sb.AppendLine($"- resolution: {info.Width}x{info.Height} @ {Bicim.Tani.Kare(info.Fps)} fps");
        sb.AppendLine($"- video codec: {info.VideoCodec}, pixel format: {info.PixelFormat ?? "unknown"}, HDR: {(info.IsHdr ? "yes" : "no")}");
        sb.AppendLine($"- current size: {Bicim.Tani.Mb(info.FileSizeMb)} MB, total bitrate: {info.TotalBitrateBps / 1000} kbps");
        sb.AppendLine(info.HasAudio
            ? $"- audio: {info.AudioCodec}, {info.AudioBitrateBps / 1000} kbps, {info.AudioChannels} channel(s)"
            : "- audio: none");
        sb.AppendLine();
        sb.AppendLine("GOAL");
        sb.AppendLine($"- target size: {Bicim.Tani.Hedef(options.TargetMb)} MB (hard ceiling)");
        ...
        sb.AppendLine($"- {localPlan.Codec} / {localPlan.Mode} / {localPlan.VideoBitrateK}k video / {localPlan.AudioBitrateK}k audio / {localPlan.Width}x{localPlan.Height} @ {Bicim.Tani.Kare(localPlan.Fps)} / preset {localPlan.Preset}");
        sb.AppendLine();
