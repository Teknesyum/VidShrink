...
Karar ver (ucuncu tur, sorular cevaplandi): VidShrink oynatici motoru icin on adayi puanlayan tablo ve en mantikli secim; kendi oynaticimizi yazmanin iki yolunun (boru genisletme, FFmpeg.AutoGen surec ici) Standart 30 + Gelismis 13 maliyeti libmpv ile karsilastirmali; 004 kararini koru ya da degistir, degisirse dalga plani.

## Elde olan olgular

Netleştirme 006'nın iki sorusuna T0'ın cevapları:
1. İkisi de ayrı satır olarak maliyetlenir: (a) bugünkü ffmpeg-süreç kodunu genişletmek (aday 5), (b) FFmpeg.AutoGen ile süreç içi sıfırdan (aday 4). Kullanıcının 'kendimizinkini yazmak' dediği ikisini de kapsıyor.
2. macOS karar sütunudur ama ağırlığı Windows'tan düşük (Windows birincil, macOS ikincil). Paketleme riski o sütunda derecelenir.

Bu üçüncü tur; 005 ve 006'nın bütün soruları cevaplandı. Yeni netleştirme sorusu sorma: kalan belirsizlikte varsayımını tek satırla yazıp kararın tamamını döndür (tablo, seçim, kendi yazımın iki satırlık maliyeti, 004'ün korunup korunmadığı, değiştiyse dalga planı). Cevap Türkçe.

Önceki turların olguları (006 ile aynı, kısaltılmadan):
