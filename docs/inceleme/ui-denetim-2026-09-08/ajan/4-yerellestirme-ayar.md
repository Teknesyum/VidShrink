...
- `App.axaml:5` + `Themes/Theme.axaml` — `RequestedThemeVariant="Dark"` sabit, `ThemeDictionaries`/açık palet yok, sistem teması takibi yok; tek palet olduğu için açık/koyu çift karşılaştırması yapılamadı — tasarım kararıysa AGENTS.md'ye yaz, değilse `Default` + `ThemeDictionaries`.
