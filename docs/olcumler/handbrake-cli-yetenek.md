# HandBrakeCLI Yetenek Dökümü

**HandBrake 1.11.2**, 18 Eylül 2026, bu makinede kurulu sürümün kendi çıktısı.
Rapora giren "HandBrake şunu yapıyor / yapmıyor" cümlesi bu dökümden doğrulanır;
başka sürümde geçerli değildir.

## Sürüm ve donanım algısı

```
[06:27:01] Compile-time hardening features are enabled
[06:27:01] nvenc: version 13.1 is available
[06:27:01] nvdec: is available
[06:27:01] CUDA Version: 12.0
[06:27:01] VCE: Init Failed
[06:27:01] vcn: not available on this system
[06:27:01] qsv: not available on this system
[06:27:01] hb_init: starting libhb thread
[06:27:01] thread 2 started ("libhb")
HandBrake 1.11.2
...
Üç satır, yukarıdaki dökümden:

- `psy-rd`, `psy-rdoq` ve uyarlamalı niceleme HandBrake'te **üst düzey anahtar değil**;
  `--help` içinde geçmiyorlar. Kodlayıcıya `-x, --encopts` ile geçiyorlar (satır 172).
  Yol haritasındaki "argümanlarımıza girmemesinden geliyor" cümlesi bu yüzden
  HandBrake'in kendi varsayılan `--encoder-preset` değerlerini de kapsıyor: biz
  o anahtarları yazmazken HandBrake x265 ön ayarının içinden alıyor.
- `--encoder-preset` ve `--encoder-tune` kodlayıcıya göre değişiyor (satır 160-171);
  kıyasta hangi değerin kullanıldığı yazılmadan sayı tekrarlanamaz.
- Bu makinede NVENC 13.1 ve NVDEC var, VCE yok (sürüm bölümü). Donanım yolu
  kıyası bu makinede yalnız NVENC kolunda anlamlı.
