...
| D1 donanım bütçe doldurma | **girdi, kısmen (23 Eylül)** | `BudgetFill.cs:9` `NvencAim` 0,97 → 0,985; yerel NVENC 18 hücre: ortalama teslim 0,9750 → 0,9780 (boşluk %2,50 → %2,20), hedefi aşan teslim 0/18, `docs/olcumler/d1-donanim-dolum.md`. Kalan boşluğun çoğu tavan koruması yolunda (parlak/hevc/1000 0,874) ve NVENC'in küçük hedefte isteğe yanıt vermemesinde (karanlık/av1/1000); ölçülmedi |
...
| #36 otomatik kırpma | **girdi (arayüz, 23 Eylül)** | Gelişmiş > süzgeç paneli "Siyah bantları kırp" kutusu (`MainWindow.axaml:755`, varsayılan kapalı); kaynak başına bir `CropProbe` yoklaması önbellekte (`MainWindow.OtomatikKirpma.cs`), `OtomatikKirpma.Uygula` elle yazılmış `crop=`'a dokunmaz (`MainWindow.axaml.cs:2225`), başlatmada yoklama beklenir (`:4172`), gerekçe satırı `main.reason.auto-crop` (`MainWindow.Gerekce.cs:132`); kuyruk bayrağı taşır, her dosya kendi yoklamasıyla (`ShrinkJobWindow.axaml.cs:179`, `:489`); `OtomatikKirpmaArayuzTests` 6/6, `WithCrop` mutasyonu 2 kırmızı |
