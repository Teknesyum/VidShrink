...
| B3 VT plan yolu / VP9 küçültme | VP9 **dalda** (`worktree-agent-a67ad4bb287086d58`); VT yapılmadı | `AllowedCodecs`'te `*_videotoolbox` yok. VP9: `-b:v` + `-pass 1/2`, `-deadline good -cpu-used 4 -row-mt 1`, kap WebM, ses opus; WebM'in taşımadığı iz `WebmStreamDropped` notuyla düşer; HandBrake `av_webm`/`VP9` artık gerçek kol (kilit `libvpx-vp9`). cpu-used ve vp9 CRF ölçeği ölçülmedi. `Vp9KucultmeTests`, mutasyonlar `docs/olcumler/vp9-kucultme-mutasyonlar.md` |
